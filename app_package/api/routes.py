...
def receive_api_calls():
    ...
    logger_api.info(f"- Accessed posts endpoint -")
    ...
    logger_api.info(f"- Received call from {request_ip} -")
    if request_headers.get('password') == current_app.config.get('DESTINATION_PASSWORD'):
        logger_api.info(f"- password verified -")
        dict_name  = request_ip + "," + request_datetime.strftime("%Y%m%d_%H%M_%S_%f")
        ...
        posts_dict[dict_name] = {
            ...
            "date_time":request_datetime.strftime("%Y%m%d_%H%M_%S")
            }
        ...
        logger_api.info(f"{posts_dict}")
        logger_api.info(f"- data stored-")
